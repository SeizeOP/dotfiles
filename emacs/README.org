...
#+begin_src emacs-lisp
  (hd/leader-keys
    ...
    "f f" '(query-replace-regexp :wk "Edit Emacs config")
    ...
    "f w" '((lambda () (interactive) (find-file "~/.config/waybar/waybar.org")) :wk "Edit Waybar config")
#+end_src
...
** Treemacs 
Start treemacs in /Evil/ insert mode, and provide local keybind overrides for next and previous lines.
#+begin_src emacs-lisp
...
** HTML Export Control                                            :noexport:
Section containing tags pertaining to how this document shoud be formatted when exporting to HTML. 
