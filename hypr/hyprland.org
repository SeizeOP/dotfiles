...
Normal this script would run once an hour via =cron=; however, at times I will login to my sistem inbetween one of these hourly checks. Therefor it is necessary to run the =bluelight= once upon login to proper set the =hyprsunset= value at /Hyprland/ startup.
